...
- `overlay` 上 `wheel`（`passive: false`）：目标不在 `.settings-body` 内 → `preventDefault()`；滚动键（空格/翻页/方向键）同理，但空格对 `button/a/input/[role=button]` 是激活键，放行。
- `overlay` 上还要拦 `touchmove`（`passive: false`），同样是"目标不在 `.settings-body` 内就 `preventDefault`"。
...
- **互斥关系**：`main.js` 的整页接管 `isActive` 里排除"设置面板已打开"，并用 `skip` 放行落在 `.playlist-inner` / `.settings-body` 内的滚轮（这两处各有自己的接管器）；设置面板的接管器 `isActive` 只管面板是否打开。
...
- **播放列表面板高度与标签栏联动**：`.music-playlist.open` 的 `max-height` 已按标签栏 + 提示条高度预留 `42px`
  （`.playlist-tabs` 与 `.playlist-local` 固定在滚动区之上、不滚动）。增删标签栏内容时须同步这个数值与两个媒体查询内的覆写值，
  否则列表会被压扁或撑破卡片。
