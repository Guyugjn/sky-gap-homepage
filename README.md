...
- **日夜双模星座查询** — 白天用塔罗牌卡日历（12 张月份符卡 + 7 列日历网格），夜间用旋转星环日轨（双环星盘持续自转，点星选月日）。选齐后光束射向星图、星座连线金色逐笔浮现。
- **藏星于野的星图** — 12 黄道星座按经典星图画法藏在繁星里，平时与普通星点无异；确认生日后，属于你的星座被金线一笔一笔勾勒浮现。
- **Live2D 看板娘** — Pio / Izumi 双模型，120+ 条对话与页面元素联动。延迟初始化不抢首屏带宽，移动端自动隐藏。
- **音乐播放器** — 自定义拖拽进度条/音量条，三种播放模式，智能预加载，淡入切歌。播放列表支持动量滚动（滚轮惯性滑行 + 弹簧定位），切换曲目自动跟随，点击即切歌不关闭面板。
...
- 白天模式塔罗牌卡日历（白色基底卡片 + CSS Grid 日历网格，选中蓝色亮起、确认金色锁定）
- 夜间模式星环星体（径向渐变亮核光晕、随机相位呼吸闪烁、选中星四芒星光、轨道虚线流动、碎屑光点漂移）
- 三层云朵 CSS 漂移 + 鼠标视差
...
js/zodiac.js         — 星座数据、运势 API、白天塔罗牌卡日历、夜间星环日轨、星空 Canvas、动效编排、滚动浮现
